...
void main()
{
   ...
   vec3 tmpr = 2.0 * NoV * Normal - ViewDirection;
   vec3 InDirectLightSpecularPart1 = textureLod(IBL_Specular_Light, tmpr , MipLevel).rgb ;

   vec2 Brdf = textureLod(IBL_LUT, vec2(NoV , Roughness), 0.0).xy ; 
   vec3 InDirectLightSpecularPart2 = SpecularColor * Brdf.x + Brdf.y ;
